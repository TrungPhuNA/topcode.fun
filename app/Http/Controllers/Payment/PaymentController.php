...
class PaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->service) $payments->where('service_code', 'like', '%' . $request->service . '%');
    ...
    {
        if ($request->service == 'vnpay')
            $this->createPaymentVnpay($request);

        $this->createPaymentNganLuong($request);
    }
    ...
    protected function createPaymentVnpay(Request $request)
    {
        ...
        $this->createPaymentTransaction($inputData);
        ksort($inputData);
        $query = "";
        $i = 0;
        $hashdata = "";
